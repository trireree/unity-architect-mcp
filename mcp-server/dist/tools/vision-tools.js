import { z } from "zod";
export function registerVisionTools(server, client) {
    // Capture Scene View
    server.tool("unity_vision_capture_scene", "Captures an instant screenshot of the active Unity SceneView camera as an image for visual inspection of lighting, layout, and composition.", {
    ...
    // Capture Game View
    server.tool("unity_vision_capture_game", "Captures an instant screenshot of the Main Game Camera view.", {
        width: z.number().optional().describe("Image width (default 1280)"),
        height: z.number().optional().describe("Image height (default 720)"),
    }, async (args) => {
        const res = await client.execute({
    ...
    server.tool("unity_vision_inspect_object", "Frames the SceneView camera directly on a specific GameObject and takes a high-res screenshot for visual inspection.", {
