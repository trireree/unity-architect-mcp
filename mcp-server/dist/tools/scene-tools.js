...
export function registerSceneTools(server, client) {
    // Get Scene Hierarchy
    server.tool("unity_scene_get_hierarchy", "Returns the complete scene hierarchy tree with GameObject IDs, tags, layers, active states, transforms, and attached components.", {}, async () => {
    ...
    // Create GameObject
    server.tool("unity_gameobject_create", "Creates a new GameObject in the active Unity scene (Empty or Primitive: Cube, Sphere, Capsule, Cylinder, Plane, Quad) with optional parent, position, rotation, and scale.", {
    ...
    // Modify GameObject
    server.tool("unity_gameobject_modify", "Modifies an existing GameObject's transform, name, tag, layer, or active state.", {
    ...
    // Delete GameObject
    server.tool("unity_gameobject_delete", "Deletes a GameObject from the scene.", {
    ...
    server.tool("unity_gameobject_duplicate", "Duplicates a GameObject in the scene.", {
