...
export function registerScriptTools(server, client) {
    ...
    server.tool("unity_script_create", "Creates or updates a C# script file in the project and triggers Unity compilation pipeline.", {
    ...
    server.tool("unity_script_get_compilation_status", "Checks if Unity Editor is currently compiling scripts or updating asset database.", {}, async () => {
    ...
    // Evaluate C# (REPL)
    server.tool("unity_csharp_eval", "Executes arbitrary C# Editor code directly in Unity without saving a file or waiting for compilation. Returns the result.", {
