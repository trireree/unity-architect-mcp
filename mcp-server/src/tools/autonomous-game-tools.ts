...
export function registerAutonomousGameTools(server: any, client: UnityClient) {
  // Phase 88: Full Autonomous Game Build
  server.tool(
    "unity_build_game",
    "Executes end-to-end autonomous game generation: Intent -> Architecture -> World Generation -> Character -> Vehicle -> AI -> UI -> Quality Gate in a single atomic transaction.",
    {
    ...
    async (args: any) => {
      ...
      if (!res.success) return { content: [{ type: "text", text: `Build Failed: ${res.error}\n${res.data || ""}` }] };
  ...
  server.tool(
    ...
    "Generates a multi-layer (World, Gameplay, AI Simulation, Presentation, Technical) game architecture blueprint with dependency graph.",
    {
  ...
  server.tool(
    ...
    "Decomposes a game concept into dependency-ordered, prioritized technical development tasks.",
    {
  ...
  server.tool(
    ...
    "Compiles natural language game requests into structured game design intent specifications.",
    {
  ...
  server.tool(
    ...
    "Generates a seed-based procedural open world with districts, asphalt road grid, buildings, and spawn anchors.",
    {
  ...
  server.tool(
    "unity_inspect_assets_v2",
    "Inspects all project asset files, texture counts, material dependencies, and sizes.",
    {},
    async () => {
  ...
  server.tool(
    ...
    "Applies automatic safe performance optimizations (static batching flags, camera far clipping) and reports draw call savings.",
    {},
    async () => {
  ...
  server.tool(
    "unity_capabilities",
    "Returns engine capabilities, verified pipelines, active Unity version, render pipeline, and system limitations.",
    {},
    async () => {
