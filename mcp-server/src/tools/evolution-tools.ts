...
export function registerEvolutionTools(server: any, client: UnityClient) {
  // Phase 21: LLM-Assisted Minimal Repair Context
  server.tool(
    "unity_repair_context",
    "Generates a minimal, token-efficient repair payload for the host LLM containing the exact error, code snippet around the line, relevant graph dependencies, and targeted Unity API knowledge to patch cleanly.",
    {},
    async () => {
  ...
  server.tool(
    "unity_analyze_impact",
    "Calculates the blast radius and risk level (LOW, MEDIUM, HIGH, CRITICAL) before modifying or deleting an asset, script, or GameObject.",
    {
  ...
  server.tool(
    "unity_run_playtest",
    "Executes automated runtime smoke test verifying object presence, attached components, active camera, and clean console without exceptions.",
    {
    ...
    async (args: any) => {
      ...
      if (!res.success) return { content: [{ type: "text", text: `Playtest Failed: ${res.error}\n${res.data || ""}` }] };
  ...
  server.tool(
    "unity_generate_city",
    "Generates a seed-based procedural city layout with roads, districts, building blocks, and player spawn points.",
    {
  ...
  server.tool(
    "unity_quality_gate",
    "Evaluates project quality score (0 - 100) combining C# compilation, scene integrity, gameplay readiness, and performance.",
    {},
    async () => {
  ...
  server.tool(
    "unity_inspect_packages",
    "Inspects Unity Package Manager manifest for key dependencies (URP, Input System, Cinemachine, TextMeshPro, AI Navigation).",
    {},
    async () => {
