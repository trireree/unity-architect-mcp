...
export function registerArchitectTools(server: any, client: UnityClient) {
  // 1. Inspect Project (Progressive Disclosure Summary)
  server.tool(
    "unity_inspect_project",
    "Returns a lightweight, token-optimized summary of the Unity Project (scene count, object count, scripts, prefabs, materials, active hash, key scene elements) without context bloat.",
    {},
    async () => {
  ...
  server.tool(
    "unity_inspect_scene",
    "Returns high-level structural overview of the active scene (scene hash, object count, root objects list).",
    {},
    async () => {
  ...
  server.tool(
    ...
    "Returns the detailed graph subtree, components, and relationships for a specific GameObject name or Stable ID.",
    {
  ...
  server.tool(
    "unity_state_diff",
    "Calculates the exact incremental diff (ADDED, REMOVED, MODIFIED, UNCHANGED) since the last state baseline. Saves enormous token context.",
    {},
    async () => {
  ...
  server.tool(
    "unity_snapshot",
    "Creates a safe checkpoint / snapshot of the current scene and assets before executing complex AI modifications.",
    {
  ...
  server.tool(
    "unity_rollback",
    "Atomically rolls back the last AI change or a specific Transaction ID, reverting scene and asset modifications.",
    {
  ...
  server.tool(
    "unity_validate",
    "Runs full integrity verification across the active scene and project (detects missing scripts, broken shaders/pink materials, missing cameras, compilation errors).",
    {},
    async () => {
  ...
  server.tool(
    "unity_execute_batch",
    "Executes multiple Unity operations sequentially in a single atomic transaction. Automatically validates integrity and rolls back on failure.",
    {
