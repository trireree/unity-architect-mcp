...
export function registerPhysicsTools(server: any, client: UnityClient) {
  // Setup Rigidbody
  server.tool(
    "unity_physics_setup_rigidbody",
    "Configures a Rigidbody physics component on a GameObject.",
    {
  ...
  // Setup Collider
  server.tool(
    "unity_physics_setup_collider",
    "Configures Box, Sphere, Capsule, or Mesh Collider on a GameObject.",
    {
      target: z.string().describe("Target GameObject name or Instance ID"),
  ...
  // Bake NavMesh
  server.tool(
    "unity_physics_bake_navmesh",
    "Bakes the NavMesh navigation data for the active scene.",
    {},
    async () => {
