...
export class UnityClient {
  ...
  public async checkHealth(): Promise<{ healthy: boolean; version?: string; error?: string }> {
    try {
      ...
      if (response.ok) {
        const data = (await response.json()) as UnityMcpResponse;
  ...
  public async execute(request: UnityBridgeRequest): Promise<UnityMcpResponse> {
    try {
      ...
      const json = (await response.json()) as UnityMcpResponse;
      return json;
    } catch (err: any) {
